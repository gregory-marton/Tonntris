...
The original plan (see "Goal Description" below) is substantially implemented and verified, and so are the fifteen rounds of follow-up work that came after it:
...
15. **Snake status message removal + continuous direction highlighting** — removed `#snake-game-status` and `setStatus()` entirely (pause state was already shown via the Pause/Resume button label; game-over/victory already have their own audio cue, so the text was redundant). Added `SnakeMode.updateDirectionHighlight()`, called whenever `nextDirection` changes (keyboard or D-pad, both go through the same keydown handler) and on reset, toggling a new `.active-direction` class onto whichever D-pad button matches the current heading so it's always visible, not just flashing on press. Closes backlog item #26 — **this closes out every item in this round's mobile-blocking backlog.**
16. **Gravity: pieces can overhang the side walls down to a single toe-hold hex** — a piece steered toward the left/right wall could previously only overhang by exactly one column (`checkActivePlacement` required every cell to sit within a fixed window just one column wider than the cup), so any piece longer than 2 cells could get stuck against an obstacle even though a genuinely open path existed further over. Real-device report: "a piece should be able to keep any single hex on the board and have enough room to move/drop, so one can get under things that are not completely blocked," confirmed with a concrete repro (an "I" piece laying flat could only hang 1 cell off either edge, never down to a single toe-hold). Fixed by changing `Board.checkActivePlacement` to allow any number of cells past the side walls as long as at least one cell keeps a toe-hold on the true playable columns (-5..4) — the floor (`q < 0`) stays a hard limit regardless. Closes backlog item #38.
17. **Sandbox: double-tap replaced with a visible place-wedge + note-play tool** — real-device testing found double-tap fundamentally flawed: rapid successive rotate-taps on the same cell (a normal, unrelated action) kept misfiring as an accidental double-tap-place, and the same shared timing state made single-tap-pickup feel inconsistent too. Rather than patch the timing window again, replaced it with a non-gesture, always-visible UI control: a "▼ place" wedge overlay on the currently-selected carousel item (mirrors Blast's existing active-item-arrow), which sidesteps the discoverability/ambiguity/accessibility problems that ruled out the alternatives considered (hold-to-place is confusable with drag and harder for slower/imprecise motor control; swipe stays as a secondary option but was never fully satisfying either). Dragging a new piece out of the carousel now also places whatever candidate was already active first, since starting a new drag is an unambiguous "I'm done with this one" signal and a wrongly-placed piece is cheap to pick back up. Since the wedge occupies most of the selected card, tapping it again is no longer an obvious way back to "nothing selected" — added a permanent, first-position carousel entry (a "♪" glyph, not text) for a dedicated note-play tool: the default mode where touching any cell, piece or not, just plays its note. This also simplified the nothing-selected tap case to *always* play the note regardless of what's on the tapped cell (previously it picked up a placed piece there), which is safer against accidentally disturbing a placement while just exploring sounds. `js/main.js`'s touchend handler lost the entire double-tap state machine (`doubleTapLastCell`/`doubleTapLastTime`/`doubleTapSnapshotRotation`) as a result.

- **11/11 unit tests pass** (`node tests/run_tests.js`)
- **164/164 Playwright tests pass** across Desktop Chrome, Mobile Chrome (Pixel 5), and Tablet Chrome (`npx playwright test`).
...
Empty as of Status item 17 below — every item that was blocking a good, consistent mobile
experience has been closed. New feature ideas (not blocking the mobile experience itself) go
in `next_steps.md` instead.
